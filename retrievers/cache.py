...
class SimpleCache:
    ...
    Note:
        This class is used by retrievers that may be called from multiple threads
        (e.g., via `QAProgram.process_batch(parallel=True)`), so access to the
        underlying in-memory dict **and** on-disk JSON file is synchronized
    ...
    def set(self, key: str, value: Any) -> None:
        """Thread-safe write to the cache (in-memory + on-disk)."""
        with self._lock:
